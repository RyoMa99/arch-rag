...
**課題**: Delete-then-Insert による更新時、ベクトルに紐づくフィードバック（信頼度スコア、Good/Bad評価）が失われる

**解決策**: 2層フィードバック管理

ベクトルストアとは別にフィードバックDB（PostgreSQL等）を設け、チャンク単位とドキュメント単位の両方でフィードバックを管理する。
...
**ポイント**:
- `chunk_hash`（チャンク内容のハッシュ）で紐付けし、更新後も同一内容ならフィードバック継承
- ドキュメント単位スコアがフォールバックとして機能し、チャンク境界変更に強い
- Rerankingでフィードバックスコアを活用（±10%のスコア調整）

**修正ファイル**:
- `docs/architecture/indexing-pipeline.md`: 信頼度管理セクション
- `docs/architecture/update-pipeline.md`: フィードバック継承フローセクション
- `docs/architecture/query-pipeline.md`: Rerankingでのフィードバックスコア活用セクション
...
- フィードバックデータの永続化により、更新時のデータ消失を防止
