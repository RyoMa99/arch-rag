...
- **信頼度管理（フィードバックループによる信頼度管理を反映）**
  ...
  - **フィードバックデータの永続化**
    - ベクトルストアとは別にフィードバックDB（PostgreSQL等）を設け、2層で管理
    - Delete-then-Insert更新でベクトルが入れ替わってもフィードバックを保持
  - **chunk_hashによる紐付け**
    - チャンク内容のハッシュ値（`chunk_hash`）をインデックス作成時に生成
    - `doc_id`, `chunk_id`, `chunk_hash`の3点セットで差分管理・フィードバック継承を実現
    - 更新後に同一`chunk_hash`が存在すれば、過去のフィードバックスコアを継承
...
- DB機能の活用
  - テナント分離・コレクション分離でホット/コールドをシンプルに実現可能
  - 独自実装の前にDB側の機能で解決できないか検討する
- 日本語対応の注意点
...
インデックス作成時のエラーとその対処方針。

| ノード | エラー種別 | 対処 |
|--------|-----------|------|
| リソース入力 | ファイル読み取りエラー | スキップしてログ記録、他ファイルは継続 |
| 前処理 | パース失敗 | 生テキストとして処理 or スキップ |
| 埋め込み生成 | API障害/タイムアウト | リトライ（最大3回）後スキップ |
| インデックス書き込み | DB接続エラー | リトライ後、キューに積んで後続処理 |

**バッチ処理のエラー方針:**
- 個別ドキュメントのエラーは全体を止めない（部分的成功を許容）
- エラー発生ドキュメントは別途キューに積み、再処理
- 致命的エラー（DB全断、ディスクフル）は即時停止してアラート

**リカバリ:**
- 処理済みドキュメントIDを記録し、再開時は未処理分から継続
- チェックポイント機能で中断点からの再開をサポート
